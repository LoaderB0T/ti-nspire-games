...
function on.charIn(ch)
	if helpactive then
		if ch == "g" then
			physiktime = physiktime - 1
			if physiktime < -1 then
				physiktime = 3
			end
			platform.window:invalidate()
		end
...
function on.paint(gc)
	...
	if helpactive then
		if physiktime <= 0 then
			gc:setColorRGB(0,0,0)
			gc:setFont("sansserif", "r", 10)
			-- Find cheat sheet here: ¯\_(ツ)_/¯
			gc:drawString("I=Q/t & E=U/d & E=F/Q & W=E*Q*s = F*s*cos(a)", 5, 5, "top")
			gc:drawString("Φ = Epot/Q = E*s & U=E*d & Q=C*U & Q=e0*E*A", 5, 25, "top")
			gc:drawString("C=e0*er*A/d", 5, 45, "top")
			gc:drawString("Millikan: Fe = Fg & QE=mg & Q=(mg)/E", 5, 65, "top")
			gc:drawString("Elektronenkanone: Ekin = Epot 1/2mv² = Q*U", 5, 85, "top")
		else
			gc:setColorRGB(0,0,0)
			gc:setFont("sansserif", "r", 10)
